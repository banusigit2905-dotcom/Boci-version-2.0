...
document.getElementById("registerForm").onsubmit = async (e) => {
    ...
    try {
        ...
        alert("Daftar Berhasil! ID: " + customId);
...
function loadResellerData() {
    db.collection("orders").where("resellerId", "==", currentUser.id).orderBy("createdAt", "desc").onSnapshot(sOrders => {
        db.collection("redemptions").where("resellerId", "==", currentUser.id).where("status", "==", "Selesai").onSnapshot(sRedeems => {
            ...
            // UPDATE: Menambahkan Tanggal di Tabel
...
function loadAdminData() {
    ...
    db.collection("orders").orderBy("createdAt", "desc").onSnapshot(snap => {
        ...
        document.getElementById("adminOrderTable").innerHTML = snap.docs.map(d => {
            ...
            // UPDATE: Menambahkan Tanggal di Tabel Admin
            return `<tr>
                <td>${formatDate(o.createdAt)}</td>
    ...
    db.collection("returns").orderBy("createdAt", "desc").onSnapshot(snap => {
        ...
        document.getElementById("adminReturnTable").innerHTML = snap.docs.map(d => {
            const r = d.data();
            return `<tr><td><b>${r.nama}</b></td><td>${r.produk}<br><i>${r.alasan}</i></td><td>${r.status === 'proses' ? `<button onclick="updateStat('returns','${d.id}')" class="btn-gold-sm">Selesai</button>` : '✅'}</td></tr>`;
        }).join('');
    });
    ...
    db.collection("complaints").orderBy("createdAt", "desc").onSnapshot(snap => {
        ...
        document.getElementById("adminCompTable").innerHTML = snap.docs.map(d => {
            const c = d.data();
            return `<tr><td><b>${c.nama}</b></td><td>${c.pesan}</td><td>${c.status === 'proses' ? `<button onclick="updateStat('complaints','${d.id}')" class="btn-gold-sm">Selesai</button>` : '✅'}</td></tr>`;
        }).join('');
    });
    ...
    db.collection("redemptions").orderBy("createdAt", "desc").onSnapshot(snap => {
        ...
        document.getElementById("adminRedeemTable").innerHTML = snap.docs.map(d => {
            const r = d.data();
            return `<tr><td><b>${r.resellerName}</b></td><td>${r.redeemName}</td><td>${r.points}</td><td>${r.status === 'proses' ? `<button onclick="updateStat('redemptions','${d.id}')" class="btn-gold-sm">Selesai</button>` : '✅'}</td></tr>`;
        }).join('');
    });
}
...
async function activateUser(uid) {
    if(confirm("Aktifkan user ini?")) { await db.collection("users").doc(uid).update({ isActive: true }); alert("User aktif!"); }
}
...
function renderSidebar() {
    ...
    // UPDATE: Menampilkan ID User di Sidebar
...
function filterProductsByCategory() {
    ...
    if(!ps) return;
    let f = catalog;
    if (cat !== "Semua") f = catalog.filter(p => (p.kategori || "Umum") === cat);
...
function loadAdminCatalog() {
    ...
    if(!table) return;
    table.innerHTML = catalog.map(p => `<tr><td><b>${p.nama}</b></td><td>${p.kategori || '-'}</td><td>Rp ${p.harga.toLocaleString('id-ID')}</td><td><button onclick="deleteProduct('${p.id}')">Hapus</button></td></tr>`).join('');
}
...
function loadResellerHistory() {
    db.collection("returns").where("resellerId", "==", currentUser.id).onSnapshot(s => {
        document.getElementById("resellerReturnHistory").innerHTML = s.docs.map(doc => {
            const d = doc.data();
            return `<tr><td><b>${d.produk}</b></td><td>${d.status || 'proses'}</td></tr>`;
        }).join('');
    });
}
...
function goToRedeemStep2() { 
    ...
    if(currentPointsVal < amount) return alert("Poin tidak cukup!");
...
document.getElementById("formRedeemPoints").onsubmit = async (e) => {
    ...
    try {
        await db.collection("redemptions").add({
            ...
            redeemName: document.getElementById("redName").value, wa: document.getElementById("redWa").value, 
            points: amount, status: "proses", createdAt: firebase.firestore.FieldValue.serverTimestamp()
        });
...
document.getElementById("editProfileForm").onsubmit = async (e) => { e.preventDefault(); await db.collection("users").doc(currentUser.id).update({ nama: document.getElementById("profNama").value, hp: document.getElementById("profHp").value }); alert("Update!"); };
